...
export class QuizScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.rectangle(cx, H / 2, W, H, 0x0f0f23, 0.92).setCornerRadius(16);
    this.add.rectangle(cx, H / 2, W, H, 0x000000, 0).setStrokeStyle(2, 0x6366f1).setCornerRadius(16);
    ...
    q.options.forEach((opt, i) => {
      ...
      const bg = this.add.rectangle(cx, y, W - 60, 36, 0x1e293b, 0.8).setCornerRadius(8);
      bg.setStrokeStyle(1, 0x475569);
      bg.setInteractive({ useHandCursor: true });

      const letters = ['A', 'B', 'C', 'D'];
      const label = this.add.text(cx - (W - 60) / 2 + 14, y, `${letters[i]}.`, {
        fontSize: '13px', color: '#818cf8', fontFamily: 'monospace', fontStyle: 'bold',
      }).setOrigin(0, 0.5);

      const text = this.add.text(cx - (W - 60) / 2 + 40, y, opt, {
      ...
      bg.on('pointerdown', () => {
        ...
        // 显示正误
        const result = this.add.text(cx, H - 80, correct ? '✅ 回答正确！' : '❌ 回答错误', {
